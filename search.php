                        ...
                        <select onchange="location = this.value;">
                            ...
                            if ( ! empty( $all_cats ) && ! is_wp_error( $all_cats ) ) {
                                foreach ( $all_cats as $cat ) {
                                    echo '<option value="' . esc_url( get_term_link( $cat ) ) . '">' . esc_html( $cat->name ) . '</option>';
                                }
                            }
                            ?>
                        </select>
...
<?php else : // NORMAL SEARCH ?>
<main id="main-content" class="site-main search-page">
    <div class="container">
        
        <header class="page-header" style="margin-bottom: 40px; border-bottom: 1px solid var(--border); padding-bottom: 20px;">
            <h1 class="page-title">
                <?php printf( esc_html__( 'Hasil Pencarian: %s', 'tokoku' ), '<span>' . get_search_query() . '</span>' ); ?>
            </h1>
        </header>
        ...
        <?php if ( have_posts() ) : ?>
            <div class="search-grid" style="display: grid; gap: 20px;">
                <?php
                while ( have_posts() ) : the_post();
        ...
        <?php else : ?>
            <div class="no-results" style="padding: 40px; background: var(--bg2); border-radius: 12px; text-align: center;">
